...
void turn_off_station (byte sid, time_t curr_time);
...
static const char htmlReturnHome[] PROGMEM = "<script>window.location=\"/\";</script>\n";
...
byte findKeyVal (const char * str,
                 char *       strbuf,
                 uint16_t     maxlen,
                 const char * key,
                 bool         key_in_pgm = false,
...
void server_view_scripturl() {
    ...
    bfill.emit_p (
        PSTR (
            "<form name=of action=cu method=get><table cellspacing=\"12\"><tr><td><b>JavaScript</b>:</td><td><input type=text "
            "size=40 maxlength=40 value=\"$O\" "
            "name=jsp></td></tr><tr><td>Default:</td><td>$S</td></tr><tr><td><b>Weather</b>:</td><td><input type=text size=40 "
            "maxlength=40 value=\"$O\" "
            "name=wsp></td></tr><tr><td>Default:</td><td>$S</td></tr><tr><td><b>Password</b>:</td><td><input type=password size=32 "
            "name=pw> <input type=submit></td></tr></table></form><script src=https://ui.opensprinkler.com/js/hasher.js></script>"),
        SOPT_JAVASCRIPTURL,
...
void server_json_controller_main() {
    ...
    bfill.emit_p (
        PSTR (
            "\"loc\":\"$O\",\"jsp\":\"$O\",\"wsp\":\"$O\",\"wto\":{$O},\"ifkey\":\"$O\",\"mqtt\":{$O},\"wtdata\":$S,\"wterr\":$D,"),
        SOPT_LOCATION,
        SOPT_JAVASCRIPTURL,
        SOPT_WEATHERURL,
        SOPT_WEATHER_OPTS,
    ...
    // print sbits
    for (bid = 0; bid < os.nboards; bid++) bfill.emit_p (PSTR ("$D,"), os.station_bits[bid]);
