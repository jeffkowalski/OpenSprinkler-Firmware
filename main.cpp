...
void do_loop() {
    ...
    if (curr_time != last_time) {
                    ...
                    for (sid = 0; sid < os.nstations; sid++) {
                        ...
                        if (prog.durations[sid] && !(os.attrib_dis[bid] & (1 << s))) {
                            ...
                            if (prog.use_weather) {
                                ...
                                if (wl < 20 && water_time < 10)  // if water_percentage is less than 20% and water_time is less than
                                                                 // 10 seconds do not water
                                    water_time = 0;
                            }
        ...
        if (os.status.mas > 0) {
            ...
            for (sid = 0; sid < os.nstations; sid++) {
                ...
                if ((os.station_bits[bid] & (1 << s)) && (os.attrib_mas[bid] & (1 << s))) {
                    ...
                    if (curr_time >= q->st + mas_on_adj && curr_time <= q->st + q->dur + mas_off_adj) {
                        masbit = 1;
                        break;
                    }
                }
        ...
        if (os.status.mas2 > 0) {
            ...
            for (sid = 0; sid < os.nstations; sid++) {
                ...
                if ((os.station_bits[bid] & (1 << s)) && (os.attrib_mas2[bid] & (1 << s))) {
                    q = pd.queue + pd.station_qid[sid];
                    // check if timing is within the acceptable range
                    if (curr_time >= q->st + mas_on_adj_2 && curr_time <= q->st + q->dur + mas_off_adj_2) {
                        masbit2 = 1;
                        break;
                    }
                }
        ...
        if ((millis() / 1000) % NTP_SYNC_INTERVAL == 0)
...
void reset_all_stations() {
    ...
    for (; q < pd.queue + pd.nqueue; q++) { q->dur = 0; }
}
    ...
    switch (type) {
        ...
        case NOTIFY_STATION_OFF:
            ...
            if (os.mqtt.enabled()) {
                ...
                if (os.iopts[IOPT_SENSOR1_TYPE] == SENSOR_TYPE_FLOW) {
                    sprintf_P (payload,
                               PSTR ("{\"state\":0,\"duration\":%d,\"flow\":%d.%02d}"),
                               (int)fval,
                               (int)flow_last_gpm,
        ...
        case NOTIFY_FLOWSENSOR:
            ...
            if (ifttt_enabled) {
                sprintf_P (postval + strlen (postval),
                           PSTR ("Flow count: %d, volume: %d.%02d"),
                           lval,
                           (int)volume / 100,
                           (int)volume % 100);
            }
