...
void OSMqtt::begin (void) {
    ...
    if (*config != 0) {
        sscanf (config,
                "\"en\":%d,\"host\":\"%" xstr (MQTT_MAX_HOST_LEN) "[^\"]\",\"port\":%d,\"user\":\"%" xstr (
                    MQTT_MAX_USERNAME_LEN) "[^\"]\",\"pass\":\"%" xstr (MQTT_MAX_PASSWORD_LEN) "[^\"]\"",
                &enabled,
                host,
                &port,
                username,
...
static void _mqtt_connection_cb (struct mosquitto * mqtt_client, void * obj, int reason) {
    ...
    if (reason == 0) {
        int rc = mosquitto_publish (mqtt_client,
                                    NULL,
